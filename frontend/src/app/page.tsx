...
import { Send, Music, Disc, Settings, X, ExternalLink, Play } from "lucide-react";
interface Message {
  role: "user" | "assistant";
  content: string;
  audioUrl?: string;
  ...
  return (
    <div className="flex flex-col h-screen bg-zinc-950 text-zinc-100 font-sans overflow-hidden">
      <header className="p-4 border-b border-zinc-800 flex items-center gap-2 bg-zinc-900/50 backdrop-blur z-10 sticky top-0">
        ...
        <h1 className="font-bold text-lg tracking-tight bg-gradient-to-r from-white to-zinc-400 bg-clip-text text-transparent">Lyria Studio V2</h1>
        ...
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-zinc-500 space-y-6 animate-in fade-in zoom-in duration-500">
            ...
            <div className="text-center space-y-2">
                <p className="text-lg font-medium text-zinc-400">Describe the music you want to create</p>
                <p className="text-sm text-zinc-600">Our AI engine will compose it in seconds.</p>
            </div>
            <div className="flex gap-2">
                <button onClick={() => setInput("A synthwave track with heavy bass")} className="text-xs text-zinc-400 hover:text-white bg-zinc-900 hover:bg-zinc-800 px-4 py-2 rounded-full border border-zinc-800 transition-colors">
                    "Synthwave track"
                </button>
                <button onClick={() => setInput("Lo-fi hip hop beat for studying")} className="text-xs text-zinc-400 hover:text-white bg-zinc-900 hover:bg-zinc-800 px-4 py-2 rounded-full border border-zinc-800 transition-colors">
                    "Lo-fi beat"
                </button>
            </div>
          </div>
        ...
        {messages.map((msg, idx) => (
          ...
          >
            <div
              ...
              `}
            >
              <p className="text-sm mb-3 font-medium leading-relaxed opacity-90">{msg.content}</p>
              
              {msg.audioUrl && (
                <div className="mt-4 bg-zinc-950/40 p-4 rounded-2xl border border-zinc-800/50 backdrop-blur-sm">
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center">
                        <Play size={14} className="text-blue-400 ml-0.5" fill="currentColor" />
                    </div>
                    <div className="flex-1 h-8 bg-zinc-800/50 rounded-lg overflow-hidden flex items-center px-2">
                        <audio controls src={msg.audioUrl} className="w-full h-8 block opacity-80 hover:opacity-100 transition-opacity" />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-3 border-t border-zinc-800/50">
                      {msg.details && (
                          <div className="flex flex-col">
                              <span className="text-[10px] text-zinc-500 uppercase tracking-wider font-bold">Parameters</span>
                              <span className="text-xs text-zinc-400 font-mono">{msg.details}</span>
                          </div>
                      )}
                      
                      <a 
                          href={`http://localhost:8501?file=${msg.filename || ""}&prompt=${encodeURIComponent(msg.promptRef || "Imported Audio")}`}
                          target="_blank" 
                      ...
                      >
                          ...
                          <ExternalLink size={14} className="text-zinc-500 group-hover:text-blue-400 transition-colors" />
                      </a>
                  </div>
                </div>
              )}
      ...
      <div className="p-4 bg-zinc-900 border-t border-zinc-800 relative z-20">
          ...
          <div className="absolute bottom-full mb-4 left-4 right-4 sm:left-auto sm:right-4 sm:w-80 bg-zinc-900/95 backdrop-blur-xl border border-zinc-700 p-5 rounded-2xl shadow-2xl z-30 animate-in slide-in-from-bottom-4 fade-in duration-200">
            <div className="flex justify-between items-center mb-6">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-zinc-100">Generation Settings</h3>
              </div>
              <button onClick={() => setShowSettings(false)} className="text-zinc-500 hover:text-white bg-zinc-800/50 hover:bg-zinc-800 p-1 rounded-full transition-colors"><X size={14} /></button>
            </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium">
                    ...
                    <span className="text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded text-[10px]">{duration}s</span>
                </div>
                <input 
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium">
                    ...
                    <span className="text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded text-[10px]">{bpm}</span>
                </div>
                <input 
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-medium">
                    ...
                    <span className="text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded text-[10px]">{density}</span>
                </div>
                <input 
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative max-w-4xl mx-auto flex gap-3 items-end">
          <button 
            type="button" 
            onClick={() => setShowSettings(!showSettings)} 
            className={`p-3.5 rounded-2xl transition-all border shadow-sm ${showSettings ? "bg-blue-500/10 border-blue-500/50 text-blue-400" : "bg-zinc-950 border-zinc-800 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-900 hover:border-zinc-700"}`}
          >
            <Settings size={20} />
          </button>
